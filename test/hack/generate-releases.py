...
ORIGIN_RELEASES_PATH = 'release/ci-operator/config/openshift/origin'
ORIGIN_RELEASES_NAME_PATTERN = re.compile(r'^openshift-origin-release-(.+)\.yaml$')
...
class ReleaseGenerator(object):
    ...
    def _process_image_streams(self):
        for stream_version in self.releases:
            if len(self.versions) == 0 or stream_version in self.versions:
                ...
                for release_config_filename in os.listdir(RELEASE_CONFIGS_PATH):
                    ...
                    if release_version is not None and release_version == stream_version:
                        for release_stream in [stream_version, '{0}-{1}'.format(stream_version, 'art-latest')]:
                            image_stream = {
                                'kind': 'ImageStream',
                                'apiVersion': 'v1',
                                'metadata': {
    ...
    def _get_supported_releases(self):
        for filename in os.listdir(ORIGIN_RELEASES_PATH):
            match = ORIGIN_RELEASES_NAME_PATTERN.search(filename)
