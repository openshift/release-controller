...
const (
	...
	jiraNightlyVersionParse      = "nightly_version_parse"
	jiraNightlyPrevReleaseCalc   = "nightly_previous_release_calculation"
)
...
func initializeJiraMetrics(*prometheus.CounterVec) {
	...
	jiraErrorMetrics.WithLabelValues(jiraNightlyVersionParse).Add(0)
	jiraErrorMetrics.WithLabelValues(jiraNightlyPrevReleaseCalc).Add(0)
}
...
func (c *Controller) syncJira(key queueKey) error {
	...
	if verifyIssues == nil && strings.HasSuffix(release.Config.Name, ".nightly") {
		...
		if err != nil {
			klog.V(2).Infof("jira error: cannot parse version from nightly release name %s: %v", release.Config.Name, err)
			c.jiraErrorMetrics.WithLabelValues(jiraNightlyVersionParse).Inc()
			return fmt.Errorf("jira error: cannot parse version from nightly release name %s: %w", release.Config.Name, err)
		}
		// Try to calculate the previous release tag
		calculatedTag, err := c.calculatePreviousReleaseTagForNightly(version)
		if err != nil {
			klog.V(2).Infof("jira error: unable to auto-calculate previousReleaseTag for nightly release %s: %v", release.Config.Name, err)
			c.jiraErrorMetrics.WithLabelValues(jiraNightlyPrevReleaseCalc).Inc()
			return fmt.Errorf("jira error: unable to auto-calculate previousReleaseTag for nightly release %s: %w", release.Config.Name, err)
		}
