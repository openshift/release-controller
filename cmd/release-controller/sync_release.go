...
func (c *Controller) ensureReleaseMirrorJob(release *releasecontroller.Release, name string, mirror *imagev1.ImageStream) (*batchv1.Job, error) { //nolint:unused
	mirrorJobName := releaseMirrorJobName(name)
	return c.ensureJob(mirrorJobName, nil, func() (*batchv1.Job, error) {
		...
		job, prefix := newReleaseJobBase(mirrorJobName, cliImage, release.Config.AlternateImageRepositorySecretName)
